...
internal class StorageOverlayScreen: Screen(Component.literal("Storage Overlay")) {
    ...
    private fun syncContainerBounds() {
        ...
        init(NoammAddons.mc, Resolution.width.toInt(), Resolution.height.toInt())
    ...
    fun renderContainerOverlay(context: GuiGraphics, delta: Float, mouseX: Int, mouseY: Int) {
        val screen = containerScreen ?: return
        Resolution.refresh()
        syncContainerBounds()
        Resolution.push(context)
        ...
        drawPages(context, scaledMouseX, scaledMouseY, delta, activeSlot, chestSlots)
        drawScrollBar(context)
        drawPlayerInventory(context, scaledMouseX, scaledMouseY)
        Resolution.pop(context)
    }
    ...
    fun mouseClickFromContainer(click: MouseButtonEvent, doubled: Boolean) =
        mouseClicked(click.x(), click.y(), click.button(), click.modifiers(), (handler as? StorageBackingHandle.Page)?.storagePageSlot)

    fun mouseDraggedFromContainer(click: MouseButtonEvent, deltaX: Double, deltaY: Double) = mouseDragged(click.y())

    fun mouseScrolledFromContainer(mouseX: Double, mouseY: Double, verticalAmount: Double) = mouseScrolled(
