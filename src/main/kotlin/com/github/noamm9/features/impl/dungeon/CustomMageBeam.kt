...
object CustomMageBeam: Feature("Renders a fully custom, animated beam whenever you swing your weapon.") {
    ...
    override fun init() {
        ...
        register<TickEvent.End> {
            if (! mc.options.keyAttack.isDown) clickConsumed = false
        }
    ...
    private fun isValidMobTarget(entity: Entity): Boolean {
        ...
        return hasHealthTag(entity)
    }

    private fun hasHealthTag(entity: LivingEntity): Boolean {
        entity.customName?.formattedText?.let { if (it.endsWith("§c❤")) return true }

        val pos = entity.position()
        val searchBox = AABB(pos.x - 1.0, pos.y - 0.5, pos.z - 1.0, pos.x + 1.0, pos.y + 3.0, pos.z + 1.0)

        return entity.level().getEntities(entity, searchBox) { it is ArmorStand }.any { stand ->
            stand.customName?.formattedText?.endsWith("§c❤") == true
        }
    }
    ...
    private fun tryTrigger() {
        ...
        if (mc.player?.mainHandItem?.skyblockId in EXCLUDED_ITEM_IDS) return
