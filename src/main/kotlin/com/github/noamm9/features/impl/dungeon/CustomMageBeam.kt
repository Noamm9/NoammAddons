    ...
    override fun init() {
        ...
        register<RenderWorldEvent> {
            ...
            for (beam in beams) {
                ...
                else renderCustomBeam(event.ctx, beam, partialTick)
            }
        }
    }

    private fun renderCustomBeam(ctx: RenderContext, beam: Beam, partialTick: Float) {
        val direction = beam.max.subtract(beam.min).normalize()
        val length = beam.min.distanceTo(beam.max).toFloat()
        val (grow, fade) = envelope(beam, partialTick)
        val animTime = (DungeonListener.currentTime - beam.startTick + partialTick) / 20f
        BeamRenderer.render(ctx, beam.min, direction, buildStyle(length), animTime, grow, fade, beam.hashCode())
    }

    private fun envelope(beam: Beam, partialTick: Float): Pair<Float, Float> {
        val now = DungeonListener.currentTime + partialTick
        val grow = ((now - beam.startTick) / 3f).coerceIn(0f, 1f)
        val fade = 1f - ((now - beam.lastUpdateTick) / duration.value).coerceIn(0f, 1f)
        return grow to fade
    }
