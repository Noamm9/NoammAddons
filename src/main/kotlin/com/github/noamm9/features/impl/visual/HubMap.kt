...
object HubMap: Feature("Replaces the SkyBlock Hub world map with a custom image.") {
    ...
    private val imageUrl by TextInputSetting("Image URL", "").withDescription("Direct link to an image (PNG/JPEG/GIF/BMP). Preferred resolution: ${COLUMNS * TILE_SIZE}x${ROWS * TILE_SIZE} (${COLUMNS}:${ROWS} aspect ratio).").showIf { image.value == 1 }
    ...
    private val bindings by lazy { mapIds.withIndex().associate { it.value to it.index } }
    ...
    fun applyRenderState(mapId: MapId, state: MapRenderState, ci: CallbackInfo) {
        ...
        state.texture = locations[bindings[mapId.id] ?: return]
    ...
    private suspend fun loadFromUrl(url: String, force: Boolean) {
        ...
        if (! response.status.isSuccess()) return ChatUtils.modMessage("[$name] Failed to download (HTTP ${response.status})")
    ...
    private fun bindTexture(bytes: ByteArray) = mc.execute {
        val source = NativeImage.read(ByteArrayOutputStream().also {
        ...
        val wall = NativeImage(COLUMNS * TILE_SIZE, ROWS * TILE_SIZE, false).also { wall ->
        ...
        repeat(TILE_COUNT) {
            ...
            textures += run {
                ...
                object: DynamicTexture({ "Hub map tile $it" }, tile) {
                    init {
