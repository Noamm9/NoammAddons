...
object BeamRenderer {
    ...
    private fun drawShape(
        ...
        if (style.shape == BeamShape.RIBBON) {
            ...
            val radiusAt = { t: Float -> halfWidth * (if (style.endpointFade) 0.3f + 0.7f * taper(t) else 1f) * pulseMult }
            drawStrand(buffer, pose, ox, oy, oz, dir, right, up, style, animTime, visibleLength, passAlpha, amplitude, seed, strand = 0, sides = 2, segs = segs, radiusAt = radiusAt)
            return
        }
    ...
    private fun drawStrand(
        ...
        for (i in 0..segs) {
            ...
            val prevPoints = ring.prevPoints
            val prevColors = ring.prevColors
    ...
    private fun tubeRadius(style: BeamStyle, t: Float, animTime: Float, baseRadius: Float): Float {
        var mod = 1f
        if (style.shape == BeamShape.CYLINDER) mod *= 1f + 0.25f * sin(t * PI.toFloat() * 8f - animTime * 10f)
    ...
    private fun colorAt(style: BeamStyle, t: Float, animTime: Float, alphaMult: Float): Color {
        ...
        return when (style.colorMode) {
            ...
            ColorMode.GRADIENT -> {
                val p = style.primary; val s = style.secondary
                Color(
