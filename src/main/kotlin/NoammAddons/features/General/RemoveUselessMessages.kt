...
object RemoveUselessMessages {
	...
	private val RegexsWithFormatting = listOf(
		...
		Regex("§cThere are blocks in the way!"),
	...
	@SubscribeEvent
	 fun cancelMessage(event: PacketEvent.Received) {
		 if (!config.RemoveUselessMessages) return
		 if (event.packet !is S02PacketChat) return
		RegexsWithoutFormatting.forEach {
			if (it.matches(event.packet.chatComponent.unformattedText.removeFormatting())) {
				event.isCanceled = true
				MinecraftForge.EVENT_BUS.post(ClientChatReceivedEvent(0.toByte(), event.packet.chatComponent))
                return
            }
		}
		RegexsWithFormatting.forEach {
			if (it.matches(event.packet.chatComponent.formattedText)) {
				event.isCanceled = true
				MinecraftForge.EVENT_BUS.post(ClientChatReceivedEvent(0.toByte(), event.packet.chatComponent))
				return
			}
