...
object AutoPotion {
    ...
    fun onDungeonStart(event: ClientChatReceivedEvent) {
        if (!config.AutoPotion) return
        if (event.type.toInt() !in 0..1) return
        if (!event.message.unformattedText.removeFormatting().matches(Regex("-+\\s.+ entered.+The Catacombs, Floor [IVX]+!\\s-+"))) return
