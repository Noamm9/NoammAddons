...
object ShowItemRarity: Feature("Draws the rarity of item behind the slot") {
    @JvmField
    val drawOnHotbar = ToggleSetting("Draw on Hotbar", true)
    private val rarityOpacity = SliderSetting("Rarity Opacity", 0, 1, 0.3)
    ...
    fun onSlotDraw(stack: ItemStack?, x: Int, y: Int) {
        ...
        val color = rarity.color.withAlpha((rarityOpacity.value.toFloat()))
