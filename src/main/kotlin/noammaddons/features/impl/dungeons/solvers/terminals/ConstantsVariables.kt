...
object ConstantsVariables {
    ...
    fun getFirstClickDelay(): Long = TerminalSolver.firstClickDelay.value.toLong().takeIf { it < TerminalSolver.reSyncTimeout.value.toLong() } ?: (TerminalSolver.firstClickDelay.value.toLong() + 200)

    fun getResyncTime(): Long = TerminalSolver.reSyncTimeout.value.toLong().takeIf { it > TerminalSolver.firstClickDelay.value.toLong() } ?: (TerminalSolver.firstClickDelay.value.toLong() + 200)
