...
object TerminalNumbers: Feature("Highlights and shows the number of each term") {
    private fun BlockPos.center(): Vec3 = Vec3(x + 0.5, y + 0.5, z + 0.5)

    // todo: maybe add devs and lavers and offload to json
    ...
    private fun renderTerminal(index: Int, positions: List<BlockPos>) {
    ...
    fun onRenderWorld(event: RenderWorld) {
        if (! inBoss) return
        LocationUtils.P3Section?.let {
            terminals[it - 1].forEachIndexed(::renderTerminal)
        }
    }
}