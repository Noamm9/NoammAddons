...
object BoulderSolver {
    ...
    fun getBoulderGrid(rotation: Int): String {
        ...
        for (z in 0 .. 15 step 3) {
            for (x in 0 .. 18 step 3) {
                val block = getBlockAt(BlockPos(getRealCoord(listOf(x * 3, 66, z * 3), rotation)))
    ...
    fun onTick(event: Tick) {
        ...
        val block = getBlockAt(BlockPos(getRealCoord(relativeCoords["ironbar"] !!, roomRotation)))
    ...
    @SubscribeEvent
    fun drawSolution(event: RenderWorld) {
    ...
    fun onInteract(event: PlayerInteractEvent) {
        ...
        if (event.pos.isNull()) return
        val block = getBlockAt(event.pos) ?: return

        onBlockPlacement(block, event.pos)
    }
}
