...
object ArrowAlignSolver: Feature() {
    ...
    init {
        // Run on a timer Thread to reduce impact on preformance
        loop(50) {
            if (! config.arrowAlignSolver) return@loop
    ...
    fun onPacket(event: PacketEvent.Sent) {
        if (! config.arrowAlignSolver) return
        ...
        if (frameIndex !in clicksRemaining.keys && mc.thePlayer.isSneaking) {
