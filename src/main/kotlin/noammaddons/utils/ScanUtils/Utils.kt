...
import noammaddons.features.dungeons.BoulderSolver
...
object Utils {
	...
	fun getPuzzleRotation(): Int? {
		...
		for (i in WitherDoorsOffsets.indices) {
			...
			val block = mc.theWorld.getBlockAt(BlockPos(rx, 68, rz))
			val bottomBlock = mc.theWorld.getBlockAt(BlockPos(rx, 67, rz))
			val topBlock = mc.theWorld.getBlockAt(BlockPos(rx, 69, rz))
