# NoammAddons 1.21.11

[![GitHub Downloads](https://img.shields.io/github/downloads/Noamm9/NoammAddons/total?style=for-the-badge&logo=github)](https://github.com/Noamm9/NoammAddons/releases)
[![Discord](https://img.shields.io/discord/1281979747605418024?color=5865F2&logo=discord&logoColor=white&style=for-the-badge)](https://discord.gg/pj9mQGxMxB)

> 1.21.11 port of the greatest skyblock mod ever!

<img width="2559" height="1439" alt="image" src="https://github.com/user-attachments/assets/4185f869-e178-408d-8f28-987c9d8e1bf4" />

----

## How to install?

1. Install **[Fabric for Minecraft 1.21.11](https://fabricmc.net/use/installer/)**
2. Install **[Fabric API (0.141.3+1.21.11)](https://www.curseforge.com/minecraft/mc-mods/fabric-api/files/6187881)**
   and **[Fabric Kotlin Language (1.13.9+kotlin.2.3.10)](https://www.curseforge.com/minecraft/mc-mods/fabric-language-kotlin)**
3. Download the latest mod file from the   [**Releases page**](https://github.com/Noamm9/NoammAddons-1.21.11/releases)
4. Put the downloaded `.jar` files into your `.minecraft/mods` folder
5. Launch Minecraft using the **Fabric** profile

* Type  `/na` in chat to open the config menu.

## Contributions

Contributions are always welcome! If you have a feature idea or want to fix a bug, feel free to open an issue or submit
a pull request.

* Open an **Issue** to suggest something
* Make a **Pull Request** to add/fix stuff

---

## License

This project is licensed under the [**CC0-1.0 License**](LICENSE).
